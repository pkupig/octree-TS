...
/// @brief Structure to hold Octree-based geometric anchoring parameters for a single triangle/primitive
struct OctreeAnchor {
    // P_node: Point Cloud Mean (μ) in World Space (for L_anchor)
    CVec3f anchor_mu; 
    
    // Sigma_inv: Inverse of the 3x3 Covariance Matrix (Σ⁻¹) 
    // Stored as 6 unique elements: [Σ⁻¹_xx, Σ⁻¹_xy, Σ⁻¹_xz, Σ⁻¹_yy, Σ⁻¹_yz, Σ⁻¹_zz]
    float anchor_sigma_inv[6]; 
    
    // v_min: The minimum eigenvector (n_target) defining the target plane normal (for L_orient)
    CVec3f anchor_normal; 
    
    // Padding/Placeholder if needed for alignment, keeping it clean for now.
};
